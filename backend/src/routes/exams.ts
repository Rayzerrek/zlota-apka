...
examsRouter.openapi(createExamRoute, async (c) => {
  ...
  // neon-http driver does not support transactions, so we run sequentially.
  // The exams table has ON DELETE CASCADE on related tables, so if something
  // fails mid-way, deleting the exam cleans up all dependent rows.
  const [exam] = await db
    .insert(exams)
    .values({
      userId,
