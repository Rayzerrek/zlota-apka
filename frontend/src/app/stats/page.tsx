...
export function StatsPage() {
	const mature = CARDS.filter((c) => c.stage === "review" && c.intervalDays >= 14).length;
	const young = CARDS.filter(
		(c) => c.stage === "learning" || (c.stage === "review" && c.intervalDays < 14),
	).length;
	const news = CARDS.filter((c) => c.stage === "new" || c.stage === "due").length;
			...
			<div className="grid grid-cols-1 gap-6 pb-10 border-b border-[var(--rule)] mb-10 min-[800px]:grid-cols-[1.2fr_1fr] min-[800px]:gap-14 min-[800px]:items-end enter enter-d1">
				...
				<div className="flex flex-col gap-3">
					...
					<p className="text-[var(--ink-muted)] leading-relaxed">
						<strong className="text-[var(--ink)]">{STUDY_STATS.retentionPct}%</strong> kart oceniasz na
						poziomie 3 lub wyżej przy pierwszym podejściu, licząc z ostatnich 90 dni. Powyżej 85% — masz
						stabilną podstawę. Cel standardowy dla FSRS: <span className="mono">0.90</span>.
					</p>
					<div className="flex gap-6 pt-4 border-t border-dashed border-[var(--rule-strong)] mt-1">
						<div>
							<div className="display text-[28px] leading-none">{STUDY_STATS.streakDays}</div>
							<div className="eyebrow text-[10px]">seria</div>
						</div>
						<div>
							<div className="mono text-[28px] leading-none">{STUDY_STATS.weekMinutes}</div>
							<div className="eyebrow text-[10px]">min / tydzień</div>
						</div>
						<div>
							<div className="mono text-[28px] leading-none">{CARDS.length}</div>
			...
			<section className="enter enter-d2">
				...
				<div className="grid grid-cols-3 border border-[var(--rule)] bg-[var(--paper-2)] mb-10">
					...
					<div className="px-5 py-6 flex flex-col gap-1.5 border-l border-[var(--rule)]">
						...
						<div className="mono text-[11px] text-[var(--ink-faint)]">czekają na pierwszą powtórkę</div>
					</div>
				</div>
			</section>
				...
				<div className="flex flex-col">
					{SUBJECT_RETENTION.map((row) => (
						...
						>
							...
							<span className="mono text-[13px] text-[var(--ink)] text-right">{row.pct}%</span>
						</div>
					))}
				</div>
			</section>
				...
				<div className="flex items-center gap-2.5 mt-4 mono text-[10px] text-[var(--ink-faint)] tracking-[0.14em] uppercase">
					...
					<div className="flex gap-[3px]">
						{[
							...
							<span key={i} className="w-3 h-3 rounded-sm" style={{ background: bg }} />
						))}
					</div>
					<span>więcej</span>
				</div>
